...
% Utility functon computation for a single device, considering only the previous assignations 
   
for j=1:N  
    %  Utility Function computation for the device j
    ...
    while cont <= M % for every RAT
        ...
        E(j,cont)= EnergiapartialOD(RAT(cont),MD(j),app(j),CS1); % computation of Energy
        T(j,cont)= TempopartialOD(RAT(cont),MD(j),app(j),CS1); % computation of Time
        F_1(j,cont) = F1(S(j,cont),app(j)); % computation of F1(Str)
        F_2(j,cont) = F2(E(j,cont),app(j)); % computation of F2(E)
        F_3(j,cont) = F3(T(j,cont),app(j)); % computation of F3(T)
        U(j,cont) = app(j).c1 * F_1(j,cont) + app(j).c2 * F_2(j,cont) + app(j).c3 * F_3(j,cont); % computation of the Utility Function 
        cont = cont + 1;
    end
    % find the RAT which maximize the Utility Function
    Umax(j) = max(U(j,:));
    % choose the RAT which maximize the Utility Function
...
% compute the Utility function and S E T for the chosen RATs ratscelta

% number of connected devices for each RAT
for i = 1:M
    RAT(i).n = sum(ratscelta == i);
end
% S E T and Utility Functon computation for a single device, considering the chosen assignations 
for j=1:N
    ...
    F_2M(j) = F2(E_M(j),app(j));
    F_3M(j) = F3(T_M(j),app(j));
...
for w=1:Nloop % number of loops calculating biased randomization
    ...
    for j = 1:N
        ...
        while cont <= M
            ...
            F_1b(j,cont) = F1(S2(j,cont),app(j));
            F_2b(j,cont) = F2(E2(j,cont),app(j));
            F_3b(j,cont) = F3(T2(j,cont),app(j));
            U2(j,cont) = app(j).c1 * F_1b(j,cont) + app(j).c2 * F_2b(j,cont) + app(j).c3 * F_3b(j,cont);  
            cont = cont + 1;
        end
        % sorting of the Utility Function for the device j
    ...
    % compute the Utility function and S E T for the chosen RATs ratsceltaRND
    for j=1:N
        ...
        F_2RND(j) = F2(E_RND(j),app(j));
        F_3RND(j) = F3(T_RND(j),app(j));
...
% compute the Utility function and S E T for the chosen RATs ratsceltadef
for i = 1:M
...
if U_M_av > U_RND_av
    fprintf(1,'WORSE:                 ');
elseif U_M_av < U_RND_av
    fprintf(1,'BETTER:                ');
else
    fprintf(1,'NO IMPROVE             ');
end
fprintf('\n')
fprintf(1,'\b%.6f',(U_RND_av-U_M_av)*100/U_M_av);
fprintf(1,'%%');
fprintf('\n')
