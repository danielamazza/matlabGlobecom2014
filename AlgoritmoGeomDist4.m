...
for w=1:Nloop
    ...
    for j = 1:N
        ...
        while cont <= M
            ...
            U2(j,cont) = app(j).c1 * F_1b(j,cont) + app(j).c2 * F_2b(j,cont) + app(j).c3 * F_3b(j,cont); % funzione di costo per MD(j) se usa la RAT cont 
            cont = cont + 1;
        end
...
fprintf('\n')
if S_M_av < S_RND_av
    fprintf(1,'  BETTER: Considering Throughput, Biased RND is better:  ');
else
    fprintf(1,'  WORSE: Considering Throughput, Biased RND is worse:  ');
end
...
fprintf('\n')
if E_M_av <= E_RND_av
    fprintf(1,'  WORSE: Considering Energy, Biased RDM is worse:  ');
else
    fprintf(1,'  BETTER: Considering Energy, Biased RDM is better:  ');
end
fprintf('\n')
...
fprintf('\n')
if T_M_av <= T_RND_av
    fprintf(1,'  WORSE: Considering Time, Biased RDM is worse:  ');
else
    fprintf(1,'  BETTER: Considering Time, Biased RDM is better:  ');
end
fprintf('\n')
...
fprintf('\n')
fprintf(1,'Ref Value:  ');
fprintf(1,'\b%.2f',U_M_av); 
fprintf(1,';   Biased Rand Utility:  ');
fprintf(1,'\b%.2f',U_RND_av);
fprintf(1,';   ');
fprintf('\n')
if U_M_av >= U_RND_av
    fprintf(1,'  WORSE: Considering UF, Biased RDM is worse:  ');
else
    fprintf(1,'  BETTER: Considering UF, Biased RDM is better:  ');
end
fprintf('\n')
fprintf(1,'\b%.2f',(U_RND_av-U_M_av)*100/U_M_av);
fprintf(1,'%%');
fprintf('\n')
fprintf('\n')
